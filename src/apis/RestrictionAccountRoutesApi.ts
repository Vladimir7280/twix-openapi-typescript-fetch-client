...
export class RestrictionAccountRoutesApi extends runtime.BaseAPI {
     ...
     * Get the account restrictions
     */
    async getAccountRestrictions(address: string): Promise<AccountRestrictionsInfoDTO> {
        const response = await this.getAccountRestrictionsRaw({ address: address });
     ...
     */
    async getAccountRestrictionsMerkleRaw(requestParameters: GetAccountRestrictionsMerkleRequest): Promise<runtime.ApiResponse<MerkleStateInfoDTO>> {
        if (requestParameters.address === null || requestParameters.address === undefined) {
     ...
     * Get the account restrictions merkle
     */
    async getAccountRestrictionsMerkle(address: string): Promise<MerkleStateInfoDTO> {
        const response = await this.getAccountRestrictionsMerkleRaw({ address: address });
        return await response.value();
    }
     ...
     */
    async searchAccountRestrictionsRaw(requestParameters: SearchAccountRestrictionsRequest): Promise<runtime.ApiResponse<AccountRestrictionsPage>> {
        const queryParameters: runtime.HTTPQuery = {};
     ...
     * Search account restrictions
     */
    async searchAccountRestrictions(address?: string, pageSize?: number, pageNumber?: number, offset?: string, order?: Order): Promise<AccountRestrictionsPage> {
        const response = await this.searchAccountRestrictionsRaw({ address: address, pageSize: pageSize, pageNumber: pageNumber, offset: offset, order: order });
        return await response.value();
    }
