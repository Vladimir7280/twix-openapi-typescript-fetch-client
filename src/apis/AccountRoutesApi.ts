...
export class AccountRoutesApi extends runtime.BaseAPI {
     ...
     */
    async getAccountInfoMerkleRaw(requestParameters: GetAccountInfoMerkleRequest): Promise<runtime.ApiResponse<MerkleStateInfoDTO>> {
        if (requestParameters.accountId === null || requestParameters.accountId === undefined) {
     ...
     * Get account merkle information
     */
    async getAccountInfoMerkle(accountId: string): Promise<MerkleStateInfoDTO> {
        const response = await this.getAccountInfoMerkleRaw({ accountId: accountId });
        return await response.value();
    }
