...
export class MosaicRoutesApi extends runtime.BaseAPI {
     ...
     */
    async getMosaicMerkleRaw(requestParameters: GetMosaicMerkleRequest): Promise<runtime.ApiResponse<MerkleStateInfoDTO>> {
        if (requestParameters.mosaicId === null || requestParameters.mosaicId === undefined) {
     ...
     * Get mosaic merkle information
     */
    async getMosaicMerkle(mosaicId: string): Promise<MerkleStateInfoDTO> {
        const response = await this.getMosaicMerkleRaw({ mosaicId: mosaicId });
        return await response.value();
    }
